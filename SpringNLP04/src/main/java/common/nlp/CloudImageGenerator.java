...
//    public static final String FONT_FAMILY = "나눔명조";
    ...
    public BufferedImage generateImage(Iterable<WordCount> words, long seed) {
        ...
        while (iter.hasNext()) {
            ...
            if (wc.getCnt() < MINIMUM_WORD_COUNT) break;
            int prevK = k;
            if (k > MINIMUM_FONT_SIZE) k = k - FONT_STEP_SIZE;
            Font font = new Font(fontFamily, Font.BOLD, k);
            g.setFont(font);
            FontMetrics fm = g.getFontMetrics();
            Shape s = stringShape(font, fm, wc.getWord(), rand);
            boolean fitted = false;
            for (int i = 0; i < REJECT_COUNT * wc.getCnt(); i++) {
                s = stringShape(font, fm, wc.getWord(), rand);
