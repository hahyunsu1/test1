...
/**
 * init() : 클라이먼트 첫 요청이 있을 때 딱 한번만 호출되는 매서드
 * service() : 스레드에 의해 호출되는 매서드.
 * 		- 요청방식이 get방식이면 이 안에서 doGet()을 호출한다.
 * 		-		 post			doPost()
 *		-		 delete			doDelete()
 *		-		 put			doPut()
 *destroy() : 서버가 shutdown될 떄 딱 한번 호출되는 매서드.
 *			  정상종료될 떄 딱 한번 호출함
 */
