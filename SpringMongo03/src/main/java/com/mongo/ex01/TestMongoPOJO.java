...
import static com.mongodb.client.model.Filters.*;//eq(),gt(),lt(),gte()...
import static com.mongodb.client.model.Updates.*;//combine(),set()...
...
public class TestMongoPOJO {
	...
	* */
	...
	private void mappingPojo() {
		...
		//몽고디비에서 가져온 BSon데이터를 자바 POSJO로 인코딩,디코딩하도록 설정하는 작업
		// Bson문서를 Java Pojo객체에 매핑하는 메서드==> 코덱 레지스트리가 필요함
		...
		//몽고 클라이언트 객체를 생성한뒤 이를 통해 몽고데이터베이스를 얻어온다.
