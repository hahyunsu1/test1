...
{bid:freeboard_id,title:'내일은 즐거운 주말입니다',content:'역시 좋은 시간 되세요',writer:'박해피',
    comment:[
        ...
        {writer:'최철수',cotent:'좋은 시간 되세요'}
    ]}
)
db.article.deleteOne({title:''})
...
db.article.find({comment:{$size:3}})
...
//select author, sum(score) as total from article
db.articles.aggregate([{
    $group:{_id:'$author',total:{$sum:'$score'}}
}])
//select author, sum(score) as total from article group by author having sum(score)>100
db.articles.aggregate([{
    $group:{_id:'$author',total:{$sum:'$score'}}
},{$match:{total:{$gt:100}}}])
//$sum
