...
//select sum(price) as totalPrice from orders by cust_id
db.orders.aggregate([{
    $group:{
        _id:"$cust_id",
...
//select cust_id,ord_date,sum(price) from orders group by cust_id, ord_dae
db.orders.aggregate([{
   $group:{
       _id:{
           cust_id:'$cust_id',
...
//select status,sum(price) from orders group by status
db.orders.aggregate([{
    $group:{
        _id:"$status",
        totalprice:{$sum:"$price"}
...
db.items.aggregate([{
    $group:{_id:{date:{$dateToString:{format:"%Y-%m-%d", date:"$date"}}}, 
     ...
     avgQty:{$avg:"$quantity"},count:{$sum:1}}
