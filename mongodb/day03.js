...
//dim_cm 중 인덱스가 1번쨰인 값이 10보다 큰 상품들을 조회해서 출력하셍
db.inventory.find({"dim_cm.1":{$gt:10}})
//dim_cm 중 인덱스가 1번쨰인 값이 25보다 작은 상품들을 조회해서 출력하셍
db.inventory.find({"dim_cm.1":{$lt:25}})

//#Update
/*
    -updateOne({}) :특정 필드 값을 변경하거나, 증가,감소 시킬 떄
    -updateMany({})
    -replaceOne({}) : 도큐먼트 대체
*/
db.user.find({})
db.user.replaceOne({userid:'choi1'},{'name':'핫식스',userid:'choi2',passwd:'3333'})
db.user.replaceOne({userid:'hong'},{'name':'홍길동',userid:'hong1',passwd:'1234'},{upsert:true})
//upsert옵션을 주면 조건에 해당하는 문서가 있으면 수정하고,없으면 삽입한다.

db.user.updateOne({userid:'hong'},{$set:{passwd:1234}})
...
db.member.updateMany({userid:'Lee'},{$max:{age:30}})
...
//item이 'paper'인 상품의 tags에서 'skyblue'를 삭제하세요
...
db.article.insertMany([
    {title:"아무글이나1",writer:"ha"},
    {title:"아무글이나2",writer:"kang"},
    {title:"아무글이나3",writer:"sung"}
])

db.article.updateMany({title:'아무글이나2'},{$push:{comment:{$each:['댓글1','댓글2']}}})
db.article.find({})
db.article.insertOne({name:'noname'})